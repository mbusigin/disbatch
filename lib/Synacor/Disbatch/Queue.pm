...
sub schedule {
    ...
    #$self->logger->trace("schedule free_threads: $free_threads, maxthreads: $self->{maxthreads}, tasks_doing: " . scalar @{$self->{tasks_doing}});	# extremely verbose

    for (my $x = 0; $x < $free_threads; $x++) {
        my $document = $self->find_next_task($node);
        #$self->logger->trace("schedule found no more free tasks") unless defined $document;	# extremely verbose
        return unless $document;

        $self->logger->trace("schedule loading task: $document->{_id}");
        my $task = $self->load_task($document);

        $self->logger->trace("schedule finding a free thread for $document->{_id}");
        my $thr = $self->get_free_thread();
        if (!defined $thr) {
            $self->logger->trace("schedule no free thread found!");
            ...
            $self->logger->trace("schedule free thread does not have key 'eb' defined!");
            $self->unclaim_task($document);
            return;
        }

        $self->logger->trace("schedule starting actual task: $document->{_id}");
...
sub report_task_done {
    ...
    for (my $x = 0; $x < @{$self->{tasks_doing}}; $x++) {
        if ($self->{tasks_doing}[$x]{_id}->to_string eq $taskid) {
            ...
            $self->logger->trace("report_task_done calling schedule again after finishing task: $taskid");

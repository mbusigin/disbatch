...
//              model.addRows( [[ r["node"], r["iid"], r["status"], parameters ]] );
    ...
    showSingleTask: function( id )
    {
      ...
      req.addListener( "completed", function(e) {
        ...
        metadata.add( new qx.ui.basic.Label("" + task.iid), {row:2, column:1} );
        row1.add( metadata );
        ...
        for ( var a in task.parameters )
        {
          ...
          {
            item = new qx.ui.form.ListItem( task.parameters[a] );
            parameters.add( item );
          }
        }
      ...
      sendButton.addListener("execute", function() { 
        if (form.validate()) {
          ...
          if ( this.toggleFilterPane.getValue() )
          {
              ...
              req.setParameter( "filter", qx.util.Json.stringify(this.userFilterPaneObject.getFilter()) );
              req.setParameter( "columns", qx.util.Json.stringify(task) );
          ...
          else
          {
              ...
              req.setParameter( "object", qx.util.Json.stringify([task]) );
          }
